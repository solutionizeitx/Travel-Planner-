...
class BookingAPI(ListAPIView):
    ...
    def get_queryset(self):
        ...
        if packageId:
            qs = qs.filter(packageId=packageId)
        # if hotelId: qs = qs.filter(hotelId=hotelId)
        if hotelId:
            qs = qs.filter(hotel=Hotel.objects.get(id=hotelId))

        if typeId:
            qs = qs.filter(typeId=typeId)
        if booking_date_to:
            qs = qs.filter(booking_date_to=booking_date_to)
        if booking_date_from:
            qs = qs.filter(booking_date_from=booking_date_from)
        if number_of_people:
            qs = qs.filter(number_of_people=number_of_people)
        if user:
