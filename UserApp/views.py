...
class UserAPI(ListAPIView):
    ...
    def get_queryset(self):
        ...
        if user_name: qs = qs.filter(user_name=user_name) # check and explain
    ...
    def post(self, request):
        ...
        try:
            ...
            if id:
                print("User Updating")
                qs = UserModel.objects.filter(id=id)
                if not qs.count():
                    return ResponseFunction(0, "User Not Found",{})
                variant_obj = qs.first()
                serializer = UserModelSerializer(variant_obj, data=request.data, partial=True)
                msg = "Data updated"
            else:
            ...
            obj = serializer.save()
            return ResponseFunction(1, msg,UserModelSerializer(obj).data)
