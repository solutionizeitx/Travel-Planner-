...
class PackageAPI(ListAPIView):
    ...
    def get_queryset(self):
       ...
       # if id: qs = qs.filter(id=id) # check and explain
        ...
        if name:
            qs = qs.filter(name__icontains=name)  # check and explain
        if price:
            qs = qs.filter(price=price)  # check and explain
        if is_active:
            qs = qs.filter(is_active=is_active)
        if is_best_deals:
            qs = qs.filter(is_best_deals=is_best_deals)
        if is_trending:
            qs = qs.filter(is_trending=is_trending)
        if is_banner:
            qs = qs.filter(is_banner=is_banner)
        if type:
...
class PackageTypeAPI(ListAPIView):
    ...
    def get_queryset(self):
        ...
        if is_dropdown == '1':
            print("Drop down get request")
            self.serializer_class = PackageTypeDropdownSerializer
